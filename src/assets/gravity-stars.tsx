...
function GravityStarsBackground({
  ...
  const updateStars = React.useCallback(() => {
    if (!hasMeasuredCanvasRef.current) return;
    ...
    for (let i = 0; i < starsRef.current.length; i++) {
      ...
      if (isPointerInside && dist < mouseInfluence && dist > 0) {
      ...
      if (!isPointerInside) {
        ...
        const targetSpeed = movementSpeed * idleSpeedFactor;

        if (currentSpeed > targetSpeed && currentSpeed > 0) {
          const easedSpeed = currentSpeed + (targetSpeed - currentSpeed) * 0.05;
          const ratio = easedSpeed / currentSpeed;
          p.vx *= ratio;
          p.vy *= ratio;
        }
      }
  ...
  React.useEffect(() => {
    const handleWindowPointerMove = (event: MouseEvent) => handlePointerMove(event);
    const handleWindowTouchMove = (event: TouchEvent) => handlePointerMove(event);
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      starsRef.current.length !== starsCount
    ) {
